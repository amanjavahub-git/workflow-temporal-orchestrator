    ...
    public static void main(String[] args) {
        /**
         1)  WorkflowServiceStubs service = WorkflowServiceStubs.newLocalServiceStubs();
                    ...
                    - The newLocalServiceStubs() method used to create this instance here is appropriate
                        when the Temporal Cluster and Worker are running on the same machine
                    - WorkflowServiceStubs instance to create a Temporal Client,
                      which the Worker will use to communicate with the Temporal Cluster.

                   Note : If the Temporal Cluster is running on a different machine or if you are using
                          Temporal Cloud, you would instead call the
                          newServiceStubs(WorkflowServiceStubsOptions options)
                          method using options that include the hostname and port number used to reach its
                          frontend service.
